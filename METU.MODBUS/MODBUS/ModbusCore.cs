...
namespace METU.MODBUS.MODBUS
{
    public class ModbusCore
    {
        ...
        private const int MB_MAX_LENGTH = 1200;               //最大数据长度
        ...
        private static  byte[]? rBuff = null;                  //正确接收缓冲器volatile
        private static  byte[]? wBuff = null;                     //正确发送缓冲器volatile
        ...
        private static byte[] SendTrainCyclostyle(int node, byte[]? data, int addr, int con, byte stat)
        {
            ...
            if(data!=null)    Array.Copy(data, 0, res, headDataLen.Length, con);                   //把数据复制到数据中
...
#pragma warning restore CS8604 // 引用类型参数可能为 null。
        ...
        public static void MBDataReceive()
        {
            if (comm == null)                       //如果串口没有被初始化直接退出
                return;
            SciSchedulingLock();
            System.Threading.Thread.Sleep(10);      //等待缓冲器满
            if (!comm.IsOpen)
                return;
